...
//        prnterr("recv timeout.\n");
    ...
    ret = recvfrom(skt, szbuf, len, 0, (struct sockaddr*)&addr, &addrlen);
    prntlog("noneblock rcv %dB from %s:%d.\n",ret,inet_ntoa(addr.sin_addr),ntohs(addr.sin_port));
    return ret;
}
...
void thrdSndr(/*const char* szDestIP,ushort ushDestPort,*/ushort ushLocalPort)
{
    ...
    ret=rcvNoneblock(udpSndr,szbuf,1024,addr,100);
...
void thrdRcvr(ushort ushLocalPort)
{
    ...
    while (1)
    {
        ...
        if(nMsg==MSGASK)
        {
            ...
            prntlog("sender address: %s:%d.",szbuf,natask.uSrcPort);
            b=setNetaddr(addrRemote,szbuf,htons(natask.uSrcPort));//remote address
            assert(b);
...
{//only service for 2 LANs
    ...
    natReady natRdy;
    char szbuf[1024];
    sockaddr_in addrSndr,addrRcvr,addrSvr,addr;
